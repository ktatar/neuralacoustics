...
import os # to create dataset folder
...
mu = config['dataset_generation'].getint('mu') # damping factor, positive and typically way below 1
rho = config['dataset_generation'].getint('rho') # 'propagation' factor, positive and lte 0.5; formally defined as rho = [c*ds/dt)]^2, with c=speed of sound in medium, ds=size of each grid point [same on x and y], dt=1/samplerate
gamma = config['dataset_generation'].getint('gamma') # type of edge, 0 if clamped edge, 1 if free edge

# checkpoints
cp = 6 # num of checkpoints
...
dataset_path=config['dataset_generation'].get('path')
...
for b in range(num_of_batches):
  ...
  if (b+1) % batches_per_cp == 0 : 
    ...
    scipy.io.savemat(DATASET_PATH+file_name, mdict={'a': a.cpu().numpy(), 'u': u.cpu().numpy(), 't': sol_t.cpu().numpy()})
...
# N = 10000 # num of dataset entries
# B = 20 # batch size
...
# h = 64
# samplerate = 44100; # Hz
# nsteps = 20 # Tin = 10, T = 10 -> input [0,Tin), output [10, Tin+T)
# time_duration = nsteps/samplerate 
# print('simulation duration:', time_duration, 's')

# cp_num = 20 # num of checkpoints
# # num of checkpoints must be lower than total number of batches
# if cp_num > N//B :
#   cp_num = (N//B)//2 # a checkpoint every other batch

# # propagation params, for now common to all grid points
# # explained further below
# mu = 0.001 # damping factor, positive and typically way below 1
# rho = 0.5 # 'propagation' factor , positive and lte 0.5; formally defined as rho = [c*ds/dt)]^2, with c=speed of sound in medium, ds=size of each grid point [same on x and y], dt=1/samplerate
# gamma = 0 # type of edge, 0 if clamped edge, 1 if free edge
...
# dataset_name = 'iwe_' + 'n'+str(N) + '_' + 't'+str(nsteps) + '_' + 'w'+str(w) + '_' + 'h'+str(h) + '_' + 'mu'+MU + '_' + 'rho'+RHO + '_' + 'gamma'+GAMMA + '_0'
...
# a = torch.zeros(N, h, w)
# # solutions
# u = torch.zeros(N, h, w, nsteps)

# cnt=0
# end = N//B
# cp_rate = end//cp_num
# cp_count = 0
# cp_file = 0 # will alternate 0 and 1
...
#   sol, sol_t = irreducible_wave_equation(dev, B, p0, w, h, 1/samplerate, nsteps, mu, rho, gamma)#, exciteOn, freq, excitation_x, excitation_y)
#   # store
#   a[cnt:(cnt+B),1:h-1,1:w-1] = p0 # initial condition
#   u[cnt:(cnt+B),...] = sol # results

#   cnt += B

#   # save some checkpoints, just in case..
#   if (b+1) % cp_rate == 0 and (b+1) != end : # the last checkpoint is skipped, because it's the final complete dataset
#     #file_name = dataset_name + '_cp' + str(cp_file)  + '.mat' # first saves/updates latest checkpoint
#     file_name = dataset_name + '_cp' + str(cp_count)  + '.mat'
#     scipy.io.savemat(DATASET_PATH+file_name, mdict={'a': a.cpu().numpy(), 'u': u.cpu().numpy(), 't': sol_t.cpu().numpy()})
#     print('\tbatch', b+1, 'of', end, '(checkpoint ' + str(cp_count) + ', ' + str(cnt) + ' dataset points)')
#     cp_count += 1
#     print('\tbatch', b+1, 'of', end, '(checkpoint ' + str(cp_count) + ', ' + str(cnt) + ' dataset points'+ ', file cp' + str(cp_file) + ')')
#     #cp_file = -1*(cp_file-1)

# # save final file
# file_name = dataset_name + '.mat'
# scipy.io.savemat(DATASET_PATH+file_name, mdict={'a': a.cpu().numpy(), 'u': u.cpu().numpy(), 't': sol_t.cpu().numpy()})
# print('Dataset', file_name, 'saved in', DATASET_PATH)
# t2 = default_timer()
...
# #rvb = make_colormap([c('blue'), c('black'), 0.33, c('black'), c('white'), 0.66, c('white')])
# hdh_cmap = make_colormap([c('blue'), 0.1, c('blue'), c('black'), 0.5, c('black'), 0.5,  c('black'), c('white'), 0.9, c('white')])
# N = 10
# array_dg = np.random.uniform(0, 10, size=(N, 2))
# colors = np.random.uniform(-2, 2, size=(N,))
# plt.scatter(array_dg[:, 0], array_dg[:, 1], c=colors, cmap=hdh_cmap)
# plt.colorbar()
# plt.show()
